...
<body>
  ...
  <%
       ...
       try{ 
        ...
        PreparedStatement pst=co.prepareStatement("SELECT *  FROM request  where status1='keysent' and uemail='"+email+"' ");
    ...
    <tr>
        ...
        <td><%=rs.getString(4)%></td>
         <td><%=rs.getString(5)%></td>
          ...
          <td><a href="d2.jsp?filename=<%=rs.getString(1)%>&email=<%=rs.getString(2)%>&keyword=<%=rs.getString(3)%>&cipher=<%=rs.getString(4)%>&uemail=<%=rs.getString(5)%>&dtype=<%=rs.getString(6)%>">Download</a> </td>
